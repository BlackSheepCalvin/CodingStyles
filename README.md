...
* Mockist TDD (The way testing is done by the majority of the enterprise projects in my experience), 
* Classical TDD, 
* functional style (with little functional experience),
* data driven programming, 
* clean architecture (with little clean architecture experience)
* I also tried an observer pattern/event/reactive kind of style, but it was a little bit forced, and I was biased against it, so it didn't go well. Probably it was a bad idea to try.
...
To test this as much as possible, there will be several iterations on this task.
Check 'Experiment details.md' to read more about the planned features for versions

* In V1, each coding style variation will solve the Minimal Viable Product(MVP) in their own way. I will ignore requirements from V2 and V3 (to simulate unpredictibility). After each of them is finished, I compare these solutions.
...
* Use clean code everywhere, simplify everywhere, use SOLID, etc, but only if they really improve the codebase! - except of corse if that would break the essence of the coding style (Don't skip unit tests if you're doing TDD)
* Don't think ahead of versions! You can't prepare the code for future requirements, so in V1 you only consider V1 requirements. And in V2 only V1 and V2 requirements, etc.
* As an extension of the above, also don't make up future requirements just because you are writing a code that would work great for those requirements. (For example make up an unlikely requirement to replace part of the code, just because you just wrote something that would work really well for that requirement)
...
Check 'Scores.md' for a highly subjective summary of my experience/opinion as I work through the task with each style.
...
Some developers would maybe split this into two or even more modules, but i like to keep the number of modules as small as possible. And I don't expect Core to grow too big in the project.  
...
Each Variation has their own folder inside, with their own assembly definition file.  
They **depend** on CoreUtilities module and nothing else.  
...
It can **depend** on any other modules, nothing should depend on it.